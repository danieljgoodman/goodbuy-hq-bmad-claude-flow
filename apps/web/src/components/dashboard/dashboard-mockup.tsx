...
export default function DashboardMockup({ metrics, evaluations = [] }: DashboardMockupProps) {
  ...
  // Use your brand color system instead of hardcoded colors
  const healthScoreColor = 
    mockMetrics.healthScore >= 80 ? 'text-primary' :
    mockMetrics.healthScore >= 60 ? 'text-chart-1' : 
    'text-destructive'

  const healthScoreBg = 
    mockMetrics.healthScore >= 80 ? 'bg-primary/10' :
    mockMetrics.healthScore >= 60 ? 'bg-chart-1/10' : 
    'bg-destructive/10'
      ...
      <div className="container mx-auto px-4 max-w-7xl space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          ...
          <Card className={`relative overflow-hidden border-2 ${healthScoreBg} border-opacity-20`}>
            ...
            <CardHeader className="relative pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                ...
                Health Score
                <Info className="h-3 w-3 text-muted-foreground/70 cursor-help" title="Overall business health indicator" />
              </CardTitle>
            </CardHeader>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="w-12 h-6 flex items-end gap-px">
                  {[65, 70, 75, 78, 82].map((val, i) => (
                    <div 
                      key={i} 
                      className="bg-primary/20 dark:bg-primary/80 flex-1"
            ...
            <CardContent className="pt-0 space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1 text-sm">
                  ...
                  <span className="text-primary font-medium">+{mockMetrics.growthRate}%</span>
                </div>
                <div className="w-12 h-6 flex items-end gap-px">
          ...
          <Card>
            ...
            <CardContent className="pt-0 space-y-3">
              <div className="flex items-baseline gap-1">
                <span className={`text-3xl font-bold ${mockMetrics.growthRate >= 10 ? 'text-primary' : mockMetrics.growthRate >= 0 ? 'text-chart-1' : 'text-destructive'}`}>
              ...
              <div className="flex items-center justify-between">
                <Badge variant="secondary" className="bg-primary/10 text-primary dark:bg-primary/90 dark:text-primary-foreground">
                  Strong Growth
                </Badge>
                <div className="w-12 h-6 flex items-end gap-px">
            ...
            <CardContent className="pt-0 space-y-3">
              <div className="flex items-center gap-2">
                <span className={`text-3xl font-bold capitalize ${
                  mockMetrics.riskLevel === 'low' ? 'text-primary' : 
                  mockMetrics.riskLevel === 'medium' ? 'text-chart-1' : 
                  'text-destructive'
                }`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <Star 
                      key={val} 
                      className={`h-3 w-3 ${val <= 4 ? 'text-primary fill-current' : 'text-muted-foreground'}`} 
                    />
                  ))}
                </div>
              </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                <CardContent className="space-y-6">
                  ...
                  <Tabs value={selectedTimeframe} onValueChange={setSelectedTimeframe}>
                    ...
                    <TabsContent value={selectedTimeframe} className="space-y-4">
                      ...
                      <div className="h-64 bg-gradient-to-br from-primary/5 to-chart-1/5 rounded-lg flex items-center justify-center">
                        <div className="text-center space-y-2">
                          <BarChart3 className="h-12 w-12 text-primary mx-auto" />
                          <p className="text-muted-foreground font-medium">Interactive Charts</p>
                          <p className="text-sm text-muted-foreground">Valuation trends, health metrics, and performance indicators</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t">
                        ...
                        <div className="text-center lg:block hidden">
                          ...
                          <div className="text-sm text-muted-foreground">Customer Sat.</div>
                        </div>
                      </div>
                    </TabsContent>
                  </Tabs>
                  ...
                  {mockMetrics.totalEvaluations > 0 ? (
                    ...
                    [1, 2, 3].map((item) => (
                      <div key={item} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/90 flex items-center justify-center">
          ...
          <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/90 flex items-center justify-center flex-shrink-0">
            ...
            <Card className="bg-gradient-to-br from-primary/5 to-chart-1/5 border-primary/20">
              <CardHeader>
                <CardTitle className="text-lg text-foreground dark:text-primary-foreground">
                  💡 AI Insights
                </CardTitle>
              </CardHeader>
              <CardContent>
